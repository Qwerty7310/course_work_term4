...
LRESULT CALLBACK PieChartPageProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_PAINT:
    {
        ...
        if (flagDrawPie) DrawPieChart(hdc, ps.rcPaint); //если выбраны значения, рисуем диаграмму
            ...
            rectText.right = rect.left + (rect.right - rect.left - 270) * 3 / 4; //нижний правый
            rectText.bottom = rect.top + (rect.bottom - rect.top + 50) * 3 / 4;

            DrawText(hdc, L"Введите Ваши данные для построения круговой диаграммы.", -1, &rectText, DT_WORDBREAK | DT_CENTER | DT_VCENTER);
        }
            ...
            if (!getPieChartData()) {
                ...
                for (int i = 0; i < numPieTextBox; i++)
                    for (int j = 0; j < 2; j++) {
                        ...
                        GetWindowText(pieTextBox[i][j], pieText[i][j], length + 1); //получаем текст из текстового поля
                    }
